...
namespace FurmaLibrary
{
    public class FurmaGeneral
    {
        ...
        public double Vpolez { set; get; }//начальные значения (общие, не по каждой фурме)
        ...
        public double ReaKoks { set; get; }//начальные значения (общие, не по каждой фурме)
        ...
        public void FurmaCalc()//создаем метод расчетов значений по каждой фурме
        {
            for (int i = 0; i < Furm.Count; i++)
            {
                ...
                Furm[i].KolTepla = Furm[i].RashVodiNaF * Furm[i].Tperepad * 1000 * 4.18 / 3600;//расчет параметра для каждой фурмы(в цикле чтобы расчитать в дальнейшем среднее)
            }
            ...
            for (int i = 0; i < Furm.Count; i++)//расчет каждого параметра для каждой фурмы
            {
